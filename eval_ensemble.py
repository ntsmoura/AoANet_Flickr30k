...
vars(opt).update(
    {k: vars(infos["opt"])[k] for k in vars(infos["opt"]).keys() if k not in vars(opt)}
)  # copy over options from model


opt.use_box = max([getattr(infos["opt"], "use_box", 0) for infos in model_infos])
assert max([getattr(infos["opt"], "norm_att_feat", 0) for infos in model_infos]) == max(
    [getattr(infos["opt"], "norm_att_feat", 0) for infos in model_infos]
), "Not support different norm_att_feat"
assert max([getattr(infos["opt"], "norm_box_feat", 0) for infos in model_infos]) == max(
    [getattr(infos["opt"], "norm_box_feat", 0) for infos in model_infos]
), "Not support different norm_box_feat"
